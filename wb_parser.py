...
async def get_info_about_cards(session, url):
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        for num_page in range(1, num_pages + 1):
            url = f'https://catalog.wb.ru/catalog/electronic22/v2/catalog?ab_testing=false&appType=1&curr=rub&dest=-5818883&page={num_page}&sort=popular&spp=30&subject=515'
            task = asyncio.create_task(get_info_about_cards(session, url))
            tasks.append(task)
